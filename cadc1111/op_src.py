...
from feasibility import guarded_solve                      # noqa: E402
...
class MyOptimizer(FloorplanOptimizer):
    ...
    def solve(self, block_count, area_targets, b2b_connectivity,
        ...
        """Public entry point.  Everything goes through the hard-constraint
        guard: the evaluator scores an exception exactly like an overlap
        (cost = M = 10), and the legalizer only ever self-checks H1, so this
        is the one place that can promise the emitted layout is legal."""
        spec = rules.CaseSpec.from_evaluator(
            block_count, area_targets, constraints, target_positions)
        return guarded_solve(
            self._solve, spec, block_count, area_targets, b2b_connectivity,
